...
后端远程渠道支持 OpenAI 兼容和 Codex/中转兼容两类请求模式。Codex/中转兼容模式会通过后端代理处理流式图片结果，支持解析常见 SSE、非标准流式输出、裸 JSON、base64 图片和嵌套图片字段；对富可敌国/4router 等中转渠道已增加 HTTP/1.1、连接超时和禁用自动重试等兼容处理，减少长连接断流和重复消耗余额的问题。
